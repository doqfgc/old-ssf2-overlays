...
function runUpdate() {
	...
	if ($('#mm').get("innerHTML") != mm) {
		updating = true;
		$('.ts').animate({$left: 2520}, 2500);
        $('.dummy').animate({$$fade: 1}, 1500).then(function() {
			$('#mm').set("innerHTML", mm);
            $('.bg').animate({$$fade: 1}, 1);
            $('.bgr').animate({$$fade: 1}, 1);
            $('.bgb').animate({$$fade: 1}, 1).then(function() {
                $('.dummy').animate({$$fade: 1}, 2000).then(function() {
	...
	if ($('#p1').get("innerHTML") != p1) {
		...
		$('.players .player1').animate({$$fade: 0}, outSpeed).then(function() {
			$('#p1').set("innerHTML", p1);
			$('.players .player1').animate({$left: '565px'}, 2)
			.then(function() {
				$('.players .player1').animate({$$fade: 1, $left: '565px'}, inSpeed);
	...
	if ($('#p2').get("innerHTML") != p2) {
        $('.dummy').animate({$$fade: 0}, 1).then(function() {
            ...
            $('.players .player2').animate({$$fade: 0}, outSpeed).then(function() {
                $('#p2').set("innerHTML", p2);
                $('.players .player2').animate({$left: '1147px'}, 2)
                .then(function() {
                    $('.players .player2').animate({$$fade: 1, $left: '1147px'}, inSpeed);
                }).then(function() { updating = false; });
            });
		});
	}
	...
	if ($('#c1').get("innerHTML") != c1m) {
        $('.dummy').animate({$$fade: 0}, 2000).then(function() {
		updating = true;
   	...
   	if (g3 = 1) { //sanity check
        ...
        $('.dummy').animate({$$fade: 0}, 1).then(function() {
            ...
            $('.game1').animate({$$fade: 0}, outSpeed).then(function() {
                $('#g1').set("innerHTML", g1);
                g3 = 2;
                $('.game1').animate({$left: '1623px'}, 2)
                .then(function() {
                    $('.game1').animate({$$fade: 1, $left: '1623px'}, inSpeed);
                }).then(function() { updating = false; });
            });
            });
        }
