...
function runUpdate() {
	...
	if ($('#p2').get("innerHTML") != p2) {
		...
		$('.players .player2').animate({$$fade: 0}, outSpeed).then(function() {
			$('#p2').set("innerHTML", p2);
			$('.players .player2').animate({$left: '950px'}, 2)
			.then(function() {
				$('.players .player2').animate({$$fade: 1, $left: '925px'}, inSpeed);
			}).then(function() { updating = false; });
		});
	}
...
/*	if ($('#g1').get("innerHTML") != g1) {
		updating = true;
		stopLoupe().then(function() {
			$('#g1').set("innerHTML", g1).then(function() { updating = false; });
		});
	}
	
	if ($('#g2').get("innerHTML") != g2) {
		updating = true;
		stopLoupe().then(function() {
			$('#g2').set("innerHTML", g2).then(function() { updating = false; });
		});
	}*/
