...
function runUpdate() {
	...
	if ($('#mm').get("innerHTML") != mm) {
		updating = true;
		$('.top').animate({$top: '-50px'}, outSpeed).then(function() {
			$('#mm').set("innerHTML", mm);
	...
	if ($('#p1').get("innerHTML") != p1 || $('#p2').get("innerHTML") != p2) {
		updating = true;
		$('.players').animate({$top: '-50px'}, outSpeed).then(function() {
			$('#p1').set("innerHTML", p1);
			$('#p2').set("innerHTML", p2);
	...
	if ($('#s1').get("innerHTML") != s1 || $('#s2').get("innerHTML") != s2) {
		updating = true;
		$('.scores').animate({$top: '-50px'}, outSpeed).then(function() {
			$('#s1').set("innerHTML", s1);
			$('#s2').set("innerHTML", s2);
	...
	if ($('#gg').get("innerHTML") != gg) {
		updating = true;
		$('.btm').animate({$bottom: '-50px'}, outSpeed).then(function() {
			$('#gg').set("innerHTML", gg);
