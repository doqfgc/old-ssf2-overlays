...
function runUpdate() {
	...
	if ($('#mm').get("innerHTML") != mm) {
		updating = true;
		$('.bg').animate({$$slide: 0}, 1).then(function() {
			$('#mm').set("innerHTML", mm);
            $('.bg').animate({$$slide: 1}, 1000).then(function() {
                $('.bgwl').animate({$$fade: 1}, 250).then(function() {
	...
	if ($('#p1').get("innerHTML") != p1) {
		...
		$('.players .player1').animate({$$fade: 0}, outSpeed).then(function() {
			$('#p1').set("innerHTML", p1);
			$('.players .player1').animate({$left: '285px'}, 2)
			.then(function() {
				$('.players .player1').animate({$$fade: 1, $left: '260px'}, inSpeed);
	...
	if ($('#p2').get("innerHTML") != p2) {
        $('.dummy').animate({$$fade: 0}, 1250).then(function() {
            updating = true;
            $('.players .player2').animate({$$fade: 0}, outSpeed).then(function() {
                $('#p2').set("innerHTML", p2);
                $('.players .player2').animate({$left: '985px'}, 2)
                .then(function() {
                    $('.players .player2').animate({$$fade: 1, $left: '960px'}, inSpeed);
                }).then(function() { updating = false; });
            });
		});
	}
